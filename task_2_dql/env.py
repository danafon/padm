...
class ContinuousMazeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Clip to bounds
        collided = False
        if new_pos[0] > 1.0 or new_pos[1] < 0.0 or new_pos[1] > 1.0 or new_pos[1] < 0.0:
        ...
        if collided:
            ...
            # reward = 0
            # reward = -0.05 + np.linalg.norm(self.agent_pos - self.goal_pos) - np.linalg.norm(new_pos - self.goal_pos)
            reward = - np.linalg.norm(self.agent_pos - self.goal_pos)
            # reward = (self.goal_radius)^2/(np.linalg.norm(self.agent_pos - self.goal_pos) - 1)
